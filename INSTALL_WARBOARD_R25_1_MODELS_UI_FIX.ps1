...
Write-Host ""
Write-Host "WARBOARD R25.1 - ORK/NID MODELS + AELDARI GHOST-BASE + MISSION UI FIX" -ForegroundColor Cyan
...
$Backup = Join-Path $ProjectRoot "Library\WarboardBackups\R25_1_MODEL_UI_FIX\$timestamp"
...
'@
        ...
        # R25.1: never pass an unvalidated -1 startIndex into String.IndexOf.
        # The R25 installer could throw before its LF-format fallback ran.
        $tryResolveIndex =
            $registryText.IndexOf(
                "TryResolvePack("
            )

        if ($tryResolveIndex -lt 0) {
            throw "Could not locate TryResolvePack in ModelVisualRegistry."
        }
        ...
        if ($positionIndex -lt 0) {
            # Whitespace/line-ending tolerant fallback, constrained to the
            # TryResolvePack section so no unrelated V(...) call is changed.
            $tail =
            ...
            $positionPattern =
                'V\(\s*component\.position,\s*Vector3\.zero\s*\),'
...
Write-Host ""
Write-Host "R25.1 installed successfully." -ForegroundColor Green
Write-Host ""
