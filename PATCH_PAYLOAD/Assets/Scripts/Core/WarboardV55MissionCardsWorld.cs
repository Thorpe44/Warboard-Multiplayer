...
// WARBOARD_MISSION_CARD_ROW_R2_6
// Keeps the physical mission cards parented to the live scoreboard, but with
// much larger and more readable typography.
...
public sealed class WarboardV55MissionCardsWorld : MonoBehaviour
{
    ...
    private static void Install()
    {
        ...
        GameObject root = new GameObject("Warboard Mission Card Row R2.6");
    ...
    private Card CreateCard(string name, Vector3 localPosition, Color accent, string typeLabel)
    {
        Card card = new Card();
        ...
        CreateBlock(card.Root.transform, name + " Wood Frame", new Vector3(0f, 0f, 0.08f), new Vector3(5.18f, 5.38f, 0.13f), new Color(0.30f, 0.18f, 0.085f, 1f));
        CreateBlock(card.Root.transform, name + " Background", Vector3.zero, new Vector3(4.90f, 5.12f, 0.13f), new Color(0.050f, 0.052f, 0.058f, 1f));
        CreateBlock(card.Root.transform, name + " Accent", new Vector3(0f, 2.43f, -0.10f), new Vector3(4.78f, 0.13f, 0.055f), accent);
        CreateBlock(card.Root.transform, name + " Wooden Ledge", new Vector3(0f, -2.67f, -0.04f), new Vector3(5.30f, 0.22f, 0.34f), new Color(0.37f, 0.22f, 0.105f, 1f));

        card.Type = CreateTypeLabel(card.Root.transform, name, typeLabel, accent);

        GameObject textObject = new GameObject(name + " Text");
        textObject.transform.SetParent(card.Root.transform, false);
        textObject.transform.localPosition = new Vector3(0f, 2.10f, -0.12f);
        ...
        card.Text.fontSize = 42;
        card.Text.characterSize = 0.041f;
        card.Text.lineSpacing = 0.88f;
        card.Text.color = new Color(0.95f, 0.96f, 0.98f, 1f);

        return card;
    }

    private TextMesh CreateTypeLabel(Transform parent, string name, string typeLabel, Color accent)
    {
    ...
    private GameObject CreateBlock(Transform parent, string name, Vector3 localPosition, Vector3 localScale, Color colour)
    {
    ...
    private void FitText(TextMesh textMesh, string value)
    {
        ...
        else if (lines >= 10 || maxLine >= 42)
        {
            ...
            textMesh.characterSize = 0.031f;
        }
        else if (lines >= 8 || maxLine >= 34)
        {
            textMesh.fontSize = 39;
            textMesh.characterSize = 0.036f;
        }
        else
        {
