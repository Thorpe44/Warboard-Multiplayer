    ...
    [string]$AlreadyMarker = "")
{
    ...
    # WARBOARD_V51A_NEWLINE_SAFE_ANCHORS
    # Git on Windows can materialise project source as CRLF even when the
    # repository/API representation is LF. Match exact source semantics after
    # normalising line endings, rather than making every patch anchor depend
    # on the machine's core.autocrlf setting.
    $normalizedText =
        $Text.Replace("`r`n", "`n").Replace("`r", "`n")

    $normalizedOld =
        $Old.Replace("`r`n", "`n").Replace("`r", "`n")

    $normalizedNew =
        $New.Replace("`r`n", "`n").Replace("`r", "`n")
    ...
    $result =
        $normalizedText.Substring(0, $first) +
        $normalizedNew +
...
try
{
    Write-Host "WARBOARD v51a WINDOWS LINE-ENDING FIX - GAMEPLAY / UI BUGFIXES" -ForegroundColor Cyan
    ...
    $selfExact = Replace-ExactOnce `
        ...
        -Old "alpha`nbeta`ngamma" `
        -New "alpha`nBETA`ngamma" `
        -Label "installer CRLF/LF exact self-test"

    if ($selfExact -ne "alpha`nBETA`ngamma")
    {
        throw "Installer CRLF/LF exact replacement self-test failed."
    }
    ...
    Write-Host "[OK] v51a patch engine self-test passed." -ForegroundColor Cyan
    Write-Host ""
...
'@
    ...
    Write-Host ""
    Write-Host "WARBOARD v51a INSTALL COMPLETE" -ForegroundColor Green
